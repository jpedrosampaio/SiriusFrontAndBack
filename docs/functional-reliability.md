...
- Cargos: comparação exata após normalização tipográfica, preservando níveis e qualificadores. Nomes similares e códigos diferentes permanecem separados; pode haver duplicatas incertas, preferíveis ? perda de cargo oficial.
- Recorrência: data persistida na criação, regra ?nica em tarefas/dashboard/calendário. Mensal em 29/30/31 usa ?ltimo dia de meses menores; semanal usa dia da semana da data inicial. Dados antigos sem data usam criação no fuso de São Paulo.
