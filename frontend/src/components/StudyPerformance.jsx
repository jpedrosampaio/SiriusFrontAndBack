...
export default function StudyPerformance({ api, notebookId, onPractice }) {
  ...
  useEffect(() => {
    ...
    const load = () => axios.get(`${api}/study/notebooks/${notebookId}/learning-summary`, { signal: controller.signal })
      .then(r => { if (!controller.signal.aborted) setResult(r.data); })
      .catch(() => { if (!controller.signal.aborted) setError(true); });
    load();
    window.addEventListener('sirius-study-performance', load);
    return () => { controller.abort(); window.removeEventListener('sirius-study-performance', load); };
  }, [api, notebookId]);
